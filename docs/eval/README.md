...
`--json` emits only the report object. Task outputs are separate from the feature verdict. Evaluation is deterministic: criteria that need human judgment stay `unverifiable` and block acceptance until a real check exists. Exit 2 means evaluation completed but was not accepted; exit 3 means it could not run.

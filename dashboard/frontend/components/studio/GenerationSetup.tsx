...
export function GenerationSetup({feature, kind, busy, onCommand}: {
  ...
  const [mode, setMode] = useState<SourceMode>(kind !== "prd" && prd.published ? "prd" : "brief");
  ...
  const options: {id:SourceMode; label:string; detail:string; available:boolean}[] = [
    {id:"brief",label:"Brief and supporting context",detail:`Create the ${labels[kind]} directly from your brief and saved answers.`,available:true},
  ...
  return <form className="studio-generation-setup" onSubmit={e => {e.preventDefault(); if (selected.available && !busy) void onCommand({action:"generate",kind,source_mode:mode});}}>
    ...
    <p>{selected.detail}</p>
    {sourceLabels.length > 0 && <small>Using {sourceLabels.join(" and ")}.{newerDrafts.length > 0 && ` Publish the newer ${newerDrafts.join(" and ")} draft to include its changes.`}</small>}

...
import { ArrowLeft, ArrowRight, Check, ChevronDown, Download, FileText, GitCompareArrows, History, Layers, Loader2, MessageSquare, Pencil, Plus, Send, Sparkles, X } from "lucide-react";
...
import { CommentEditor } from "./Editors";
...
export default function Studio() {
  ...
  const [edit, setEdit] = useState<{snapshot: Snapshot; sectionTitle?: string} | null>(null);
  ...
  const reviewState = `${feature?.id}:${kind}:${doc?.head}:${feature?.revision}`;
  const reviewed = reviewedState === reviewState;
  ...
  useEffect(() => {
    ...
    const load = () => api<Feature>(`/features/${selected}`, undefined, controller.signal).then(value => {
      ...
      if (e instanceof StudioAPIError && e.status === 404) {
        setSelected(null); setFeature(null); setViewId(""); setCompare(false); setEdit(null); setError("");
  ...
  const closeEdit = useCallback(() => {setEdit(null); setViewId(""); setCompare(false);}, []), closeComment = useCallback(() => setComment(null), []);
  ...
  async function command(payload: Command): Promise<boolean> {
    ...
    try {
      ...
      if (["edit", "edit_document", "review_section", "restore", "reconcile", "revise"].includes(payload.action)) {setViewId(""); setCompare(false);}
      void loadList().catch(() => {});
      return true;
    } catch (e) {
  ...
  async function create() {
    ...
    try {
      const data = {brief:brief.trim(), context:context.trim(), kind:startKind, ...(demo ? {demo:"task-due-dates"} : {})};
      ...
      choose(next.id, next.initial_kind || startKind); setFeature(next); setBrief(""); setContext(""); setDemo(false);
    } catch (e) {setError((e as Error).message);} finally {setSaving(false);}
  }

  return <div className="studio-app">{!edit && <IconRail />}<div className="studio-shell">
    <header className="studio-topbar"><div className="studio-brand"><Link href="/define" aria-label="Back to Define" aria-disabled={Boolean(edit)} onClick={e => {if (edit) e.preventDefault();}}><ArrowLeft size={16} /></Link><span>Define</span><span className="studio-slash">/</span><strong>Authoring studio</strong><span className="studio-pill">Preview</span></div>
      <div className="studio-top-actions"><span className={`studio-connection ${connected ? "online" : ""}`} role="status"><i />{connected ? "Workspace connected" : "Connecting to workspace"}</span><button disabled={Boolean(edit)} onClick={() => choose(null)}><Plus size={14} /> New workspace</button></div></header>
    ...
    {!selected ? <main className="studio-start">
      <div className="studio-start-main"><div className="studio-eyebrow"><Layers size={16} /> GUIDED AUTHORING</div><h1>Start with a problem.<br /><span>Shape it together.</span></h1><p className="studio-intro">Start with a PRD, Design spec or RFC. Describe what you need, refine it together, and create the other documents when they help.</p>
        <div className="studio-brief surface"><fieldset className="studio-document-choice" disabled={saving}><legend>What would you like to create?</legend>{kinds.map(k => <label className={`surface ${startKind === k ? "selected" : ""}`} key={k}><input type="radio" name="starting-document" checked={startKind === k} onChange={() => {setStartKind(k); if (k === "design") setDemo(false);}} /><span><strong>{labels[k]}</strong><small>{k === "prd" ? "Define the product direction" : k === "design" ? "Shape the user experience" : "Propose the engineering approach"}</small></span></label>)}</fieldset>
          ...
          <label htmlFor="workspace-description">Describe what you want to solve</label><textarea id="workspace-description" aria-describedby="workspace-title-hint" minLength={12} maxLength={20000} required rows={5} value={brief} onChange={e => {setBrief(e.target.value); setDemo(false);}} placeholder="Describe the problem, idea or improvement. Who is affected, what happens today, and what would a better outcome look like?" />
          <p id="workspace-title-hint" className="studio-muted">We’ll create a title from your description.</p><details><summary>Have research, constraints or decisions? Add context <Plus size={12} /></summary><label htmlFor="feature-context" className="sr-only">Supporting context</label><textarea id="feature-context" maxLength={20000} rows={5} value={context} onChange={e => {setContext(e.target.value); setDemo(false);}} placeholder="Paste supporting notes. We'll distinguish your evidence from proposed assumptions." /></details>
          {demo && <p className="studio-demo-note" role="status">Saved due-date example · no AI calls. Each loading step takes about 5 seconds. Editing the description or context returns to live generation. <button type="button" onClick={() => setDemo(false)}>Use live generation</button></p>}
          <div className="studio-brief-footer"><button type="button" disabled={saving} onClick={() => {setBrief(EXAMPLE); setContext(""); setStartKind("prd"); setStartFromExisting(false); setDemo(true);}}>Use example</button><button className="primary" type="submit" disabled={saving || !connected || brief.trim().length < 12}>{saving ? <Loader2 className="studio-spin" size={16} /> : <ArrowRight size={16} />} {demo ? `Generate ${labels[startKind]}` : "Continue with brief"}</button></div></form>}</div>
      ...
      <div className="studio-feature-header"><div className="studio-feature-picker"><button aria-expanded={listOpen} disabled={Boolean(edit)} onClick={() => setListOpen(!listOpen)}><h1>{feature.title}</h1><ChevronDown size={16} /></button>{listOpen && <div className="studio-feature-menu surface">{features.map(f => <button key={f.id} onClick={() => choose(f.id, f.initial_kind || "prd")}>{f.title}{f.id === feature.id && <Check size={14} />}</button>)}<button disabled={Boolean(edit)} onClick={() => choose(null)}><Plus size={14} /> New workspace</button></div>}<p className={feature.demo ? "studio-example-label" : undefined}>{feature.demo ? "Example mode · saved content, no AI calls. Edit, compare, review and publish normally." : "Connected documents and a clear history of decisions."}</p></div>
        <div className="studio-stage-tabs" role="tablist" aria-label="Workspace documents">{kinds.map(stage => {const d = feature.documents[stage]; return <button key={stage} role="tab" aria-selected={kind === stage} disabled={Boolean(edit) || needsIntake && stage !== kind} onClick={() => switchKind(stage)}><span className="studio-stage-number">{d.published ? <Check size={12} /> : <FileText size={12} />}</span><span>{labels[stage]}<small>{needsIntake && stage === kind ? intake?.status === "ready" ? "Preparing draft" : "Clarifying brief" : d.stale.length ? "Upstream changed" : d.head ? `v${d.versions.length} · ${d.head === d.published ? "Published" : "Draft"}` : intakeFor(feature, stage) ? "Clarification started" : "Optional"}</small></span></button>;})}</div>
      </div>
      ...
      <nav hidden={Boolean(edit)} className="studio-mobile-nav" aria-label="Workspace panels"><a href="#studio-conversation">Conversation</a><a href="#studio-document">Document</a><a href="#studio-review" onClick={() => setPanel("review")}>Review</a></nav>
      <div className={`studio-workspace ${edit ? "studio-workspace-editing" : ""}`}><aside hidden={Boolean(edit)} className="studio-chat" id="studio-conversation"><div className="studio-panel-heading"><span><Sparkles size={15} /> Your writing partner</span><span className="studio-pill">{labels[kind]}</span></div>
        <div className="studio-conversation" ref={conversation} aria-live="polite" aria-relevant="additions">
          ...
          {!busy && currentQuestion && <div className="studio-question surface"><div><span className="studio-pill amber">{doc?.publication_review?.find(r => r.id === "open_questions")?.acknowledgement?.disposition === "deferred" ? "Deferred · still open" : currentQuestion.blocking ? "Decision needed" : "Worth clarifying"}</span><small>1 of {doc?.snapshot?.questions.length}</small></div><h3>{currentQuestion.question}</h3><p>{currentQuestion.why}</p><button onClick={() => {setScope(""); setMessage(`Regarding “${currentQuestion.question}”: `); composer.current?.focus();}}>Answer in chat <ArrowRight size={14} /></button></div>}
        </div>
        <form className="studio-composer" onSubmit={async e => {e.preventDefault(); if (await command({action: "revise", text: message, ...(scope ? {section_id: scope} : {})})) setMessage("");}}>
          <label htmlFor="revision-scope">Revise</label><select id="revision-scope" value={scope} onChange={e => setScope(e.target.value)} disabled={Boolean(feature.demo) || !doc?.head || busy}><option value="">Whole document</option>{doc?.snapshot?.sections.map(s => <option value={s.id} key={s.id}>{s.title}</option>)}</select>
          <label className="sr-only" htmlFor="revision-request">Revision request</label><textarea ref={composer} id="revision-request" value={message} onChange={e => setMessage(e.target.value)} rows={4} placeholder={feature.demo ? "Example mode: use Edit to change the document, or answer in the publication review below it." : doc?.head ? "Ask for a change, answer a question, or add more context…" : `Generate the ${labels[kind]} to start refining it.`} disabled={Boolean(feature.demo) || !doc?.head || busy} />
          <div><small>{feature.demo ? "Use Edit or the publication review in this example" : scope ? "Only this section will change" : "Direct edits stay protected"}</small><button type="submit" className="primary" aria-label="Send revision request" disabled={Boolean(feature.demo) || !message.trim() || !doc?.head || busy}><Send size={15} /></button></div>
        </form>
      </aside>
      <main className="studio-canvas" id="studio-document">{edit ? <DocumentEditor key={`${feature.id}-${edit.snapshot.id}`} featureId={feature.id} snapshot={edit.snapshot} sectionTitle={edit.sectionTitle} currentVersion={doc?.head} busy={busy} onClose={closeEdit} onSave={markdown => command({action:"edit_document", version_id:edit.snapshot.id, markdown})} /> : <><div className="studio-document-toolbar"><div className="studio-view-toggle"><button className={!compare ? "selected" : ""} onClick={() => setCompare(false)}><FileText size={14} /> Document</button><button className={compare ? "selected" : ""} onClick={() => setCompare(true)} disabled={!snapshot}><GitCompareArrows size={14} /> Changes</button></div><div className="studio-toolbar-actions">
        {snapshot && !isHistorical && <button aria-label="Edit document" disabled={busy} onClick={() => setEdit({snapshot})}><Pencil size={14} /> Edit</button>}
        ...
        {doc?.head && !isHistorical && (doc.published !== doc.head && (doc.blockers.length > 0 || !reviewed) ? <button aria-label={doc.blockers.length ? "Review publication blockers" : "Review before publishing"} onClick={() => {setPanel("outline"); requestAnimationFrame(() => document.getElementById("studio-publication")?.scrollIntoView({block:"nearest"}));}}>{doc.blockers.length ? `Review ${doc.blockers.length}` : "Review and publish"}</button> : <button className="primary" aria-label="Publish current snapshot" title="Publish this exact version for downstream documents" disabled={busy || doc.published === doc.head} onClick={() => void command({action: "publish", version_id: doc.head!, reviewed})}>{doc.published === doc.head ? <Check size={14} /> : "Publish"}</button>)}
      </div></div>
      ...
      <aside hidden={Boolean(edit)} className="studio-inspector" id="studio-review"><div className="studio-panel-heading"><span>{panel === "review" ? "Review comments" : "In this document"}</span>{panel === "review" && <button onClick={() => setPanel("outline")} aria-label="Close review panel"><X size={14} /></button>}</div>
        <div className="studio-inspector-scroll">{panel === "review" ? <>
          ...
          <div className="studio-publish" id="studio-publication"><h3>{doc?.published === doc?.head && doc?.head ? "Snapshot published" : "Ready to share the direction?"}</h3><p>{doc?.published === doc?.head && doc?.head ? "Downstream documents can use this exact version." : "Publish a stable version for the next document. You can keep refining afterwards."}</p>
            {doc?.head && !isHistorical && doc.publication_review && <div className="studio-publication-checklist"><p>Your review decisions apply to this version. Changes require a fresh review.</p>{doc.publication_review.map(review => <a key={review.id} href={`#publication-review-${review.id}`} onClick={() => setCompare(false)}><span>{review.title}</span><span>{review.id === "open_questions" && !review.requires_deferral ? "None found" : review.acknowledgement ? review.acknowledgement.disposition === "deferred" ? "Deferred · still open" : "Confirmed" : review.legacy_published ? "Previously published" : "Review required"}</span></a>)}</div>}
            ...
            {doc?.head && doc.published !== doc.head && !isHistorical && <>
              <label className="studio-document-review"><input type="checkbox" checked={reviewed} disabled={busy || Boolean(doc.blockers.length)} onChange={e => setReviewedState(e.target.checked ? reviewState : "")} aria-describedby="studio-document-review-help" /><span>I’ve reviewed this version</span></label>
              <p id="studio-document-review-help">{doc.blockers.length ? "Handle the items above, then review this version before publishing." : "Check the document and any saved follow-up plans before publishing."}</p>
            </>}
            <button className="primary" disabled={busy || !doc?.head || Boolean(doc?.blockers.length) || !reviewed || doc?.published === doc?.head || isHistorical} onClick={() => void command({action: "publish", version_id: doc?.head || undefined, reviewed})}><Check size={14} />{doc?.head && doc?.published === doc?.head ? "Published" : "Publish snapshot"}</button>

...
export default function Studio() {
  ...
  async function create() {
    ...
    try {
      const data = {title:title.trim(), brief:brief.trim(), context:context.trim()};
      ...
      choose(next.id); setFeature(next); setTitle(""); setBrief(""); setContext("");
  ...
  return <div className="studio-app"><IconRail /><div className="studio-shell">
    ...
    {!selected ? <main className="studio-start">
      <div className="studio-start-main"><div className="studio-eyebrow"><Layers size={16} /> GUIDED AUTHORING</div><h1>Start with the feature.<br /><span>Shape it together.</span></h1><p className="studio-intro">Turn a simple brief into a PRD you can discuss, edit and refine. Carry the agreed direction into Design and RFC.</p>
        <div className="studio-journey" aria-label="PRD to Design and RFC"><span><FileText size={16} /> PRD</span><ArrowRight size={16} /><span>Design spec</span><ArrowRight size={16} /><span>RFC</span></div>
      ...
      <div className="studio-workspace"><aside className="studio-chat" id="studio-conversation"><div className="studio-panel-heading"><span><Sparkles size={15} /> Your writing partner</span><span className="studio-pill">{labels[kind]}</span></div>
        <div className="studio-conversation" ref={conversation} aria-live="polite" aria-relevant="additions">
          ...
          {!busy && currentQuestion && <div className="studio-question surface"><div><span className="studio-pill amber">{currentQuestion.blocking ? "Decision needed" : "Worth clarifying"}</span><small>1 of {doc?.snapshot?.questions.length}</small></div><h3>{currentQuestion.question}</h3><p>{currentQuestion.why}</p><button onClick={() => {setScope(""); setMessage(`Regarding “${currentQuestion.question}”: `); composer.current?.focus();}}>Answer in chat <ArrowRight size={14} /></button></div>}
        </div>
      ...
      <div className="studio-document-scroll" ref={canvasScroll}>
        ...
        {snapshot ? compare ? compareError ? <div className="studio-loading"><p>{compareError}</p><button onClick={() => setCompare(false)}>Return to document</button></div> : snapshot.parent_id && !before ? <div className="studio-loading">Loading comparison…</div> : <VersionComparison before={before} after={snapshot} versionLabels={Object.fromEntries(kinds.flatMap(k => feature.documents[k].versions.map(v => [v.id, `${labels[k]} v${v.number}`])))} /> : <DocumentCanvas snapshot={snapshot} historical={isHistorical} busy={busy} onEdit={s => setEdit({section: s, version: snapshot.id})} onComment={(s, quote) => setComment({section: s, quote, version: snapshot.id})} onScope={id => {setScope(id); composer.current?.focus();}} onReview={id => void command({action: "review_section", section_id: id, version_id: snapshot.id})} /> : viewId ? <div className="studio-loading"><Loader2 className="studio-spin" size={20} />Loading version…</div> : <div className="studio-document-empty"><div className="studio-empty-icon"><FileText size={28} /></div><h2>{active?.kind === kind ? `Your first ${labels[kind]} is taking shape` : `Build your ${labels[kind]}`}</h2><p>{active?.kind === kind ? "We’re turning the brief into a structured draft, with assumptions and open decisions clearly marked." : kind === "prd" ? "Retry the saved request to create your first draft." : `Start with the published PRD${feature.documents.design.published && kind === "rfc" ? " and Design spec" : ""}. Then refine the details with chat, comments or direct edits.`}</p>
          {active?.kind === kind ? <div className="studio-draft-skeleton"><i /><i /><i /><i /></div> : kind !== "prd" && <button className="primary" disabled={busy || !feature.documents.prd.published} onClick={() => void command({action: "generate"})}><Sparkles size={16} /> Generate {labels[kind]}</button>}
          {kind !== "prd" && !feature.documents.prd.published && <small>Publish your PRD to continue.</small>}
        </div>}
      </div>
      </main>
      <aside className="studio-inspector" id="studio-review"><div className="studio-panel-heading"><span>{panel === "review" ? "Review comments" : "In this document"}</span>{panel === "review" && <button onClick={() => setPanel("outline")} aria-label="Close review panel"><X size={14} /></button>}</div>
        <div className="studio-inspector-scroll">{panel === "review" ? <>
          ...
          <div className="studio-publish" id="studio-publication"><h3>{doc?.published === doc?.head && doc?.head ? "Snapshot published" : "Ready to share the direction?"}</h3><p>{doc?.published === doc?.head && doc?.head ? "Downstream documents can use this exact version." : "Publish a stable version for the next document. You can keep refining afterwards."}</p>
            {doc?.head && doc.blockers.length > 0 && <details><summary>{doc.blockers.length} item{doc.blockers.length !== 1 ? "s" : ""} to review before publishing</summary><ul>{doc.blockers.map((b, i) => <li key={i}>{b}</li>)}</ul></details>}

...
| Publication review | Authors review Success, answer or defer open items, then confirm they have reviewed the document before publishing. An empty question list needs no acknowledgement. Reviews apply to one exact version and cannot bypass structural blockers. |
...
SQLite transactions provide a small, testable canonical store for the local experiment. Each feature has a monotonically increasing revision; every command supplies the expected revision. Snapshots are append-only, include exact source pins, and are retained for the lifetime of the workspace. Hashes verify snapshot bytes. Markdown export remains derived from the snapshot. The full-document editor uses a separate editable Markdown representation that is parsed and validated into the canonical structure before one atomic version is saved. A publish command selects an already stored snapshot and never rereads editable filesystem bytes.
...
The starting screen can select an existing workspace for Design or RFC and reuse the same source picker as the downstream step. It calls that workspace’s existing generate command with the displayed workspace revision and an idempotent request ID; it does not copy source content into a new workspace or create cross-workspace pins. Existing drafts and intake sessions are resumed. Optional added direction is saved in the normal operation request.
...
`edit_document` saves the entire Markdown draft against an exact snapshot ID and workspace revision in one transaction. Top-level headings preserve core section identities and support additional custom sections; Markdown tables retain stable entity IDs and references. Assumptions, questions and RFC coverage are editable, while source pins and publication evidence stay server-owned. Invalid tables, missing core headings, invalid references and stale versions reject the entire save. A no-op edit does not create a version. Only changed sections are marked protected from whole-document AI rewrites. The legacy section-edit command remains compatible for existing clients. A deliberately scoped revision can change it. Reconciliation preserves protected text and requires human review of those sections before publication. Historical revisions retain their original content and sources.

`reviews.py` derives the Success and Open questions reviews from canonical snapshot content. PRDs require a Success assessment. All document kinds require answers or a follow-up decision only when open items are detected, covering both the risks/decisions section and structured questions. Empty question lists display a status without an acknowledgement requirement. Case-insensitive text checks locate unfinished values by section, row and field. These checks help navigation; they cannot determine whether every natural-language decision is settled.

Publication review separates answering from deferring. Answers use the existing versioned revision command (scoped to Success for metric updates, or the whole document for open decisions) and include the reviewed snapshot ID. A stale reviewed version is rejected even if the workspace revision is current. Generation leaves existing manual protections intact and never automatically acknowledges or publishes the result. Review-card shortcuts open the full-document Markdown editor at the relevant section. Deferral notes remain separate review evidence.
...
Publishing checks the exact stored candidate for applicable author acknowledgements, required content, unfinished placeholders outside reviewed Success/open-question sections, unresolved RFC coverage, unresolved blocking comments, protected sections awaiting reconciliation review, unavailable source references, and stale source pins. Once these checks pass, the publish command requires `reviewed: true` alongside the exact current `version_id` and workspace revision. The publication stores the document review with its snapshot ID, timestamp and author. The UI clears the final review selection when the workspace revision changes. Review notes preserve accepted uncertainty without altering source text or clearing question flags. Multi-reviewer permissions and authenticated approval identity are not implemented in this local experiment.
...
### Offline recording example

`CreateFeature.demo = "task-due-dates"` explicitly opts a new workspace into the saved due-date scenario. Its original brief is validated; arbitrary briefs remain on the live generation path. The mode is persisted on the workspace and survives navigation, reloads and retries. `Studio.run` selects `DemoGenerator` before invoking any provider, including follow-up operations, so demo actions never fall back to AI. Each scripted operation waits five seconds outside the database transaction, then passes through the same clarification gate, validation, cancellation and stale-result checks as normal generation.

Versioned PRD/RFC fixtures are adapted from the saved due-date documents. All three selected or custom clarification answers are copied into the initial PRD requirements. Direct Markdown edits, comparisons, comments, exact-version acknowledgements and publishing use the ordinary commands. The publication-review answer action stores the author's text verbatim in the linked section; it preserves separately tracked questions and does not infer answers. It is described as a section replacement in the demo UI. General AI chat, reconciliation and Design generation are unavailable for this bounded recording scenario. The RFC records exact source pins and includes selected PRD requirements and scope without claiming to interpret arbitrary edits. Each fresh example workspace starts with no snapshots and uses new IDs.

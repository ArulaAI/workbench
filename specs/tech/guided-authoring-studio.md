...
Publication review separates answering from deferring. Answers use the existing versioned revision command (scoped to Success for metric updates, or the whole document for open decisions) and include the reviewed snapshot ID. A stale reviewed version is rejected even if the workspace revision is current. Generation leaves existing manual protections intact and never automatically acknowledges or publishes the result. Direct section editing is available from the same card. Deferral notes remain separate review evidence.

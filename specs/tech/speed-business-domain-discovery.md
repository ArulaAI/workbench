...
If either verifier rejects the candidate, permit repair requests for that semantic unit up to `semantic_repair_limit`, subject to the configured build budget and deadline. Each repair receives the original graph, the immediately previous rejected candidate, its canonical hash, all deterministic findings and its independent verification report. It returns a complete replacement candidate and an identity-change ledger. Run deterministic and independent semantic verification again on every replacement. Stop early when a replacement produces the same deterministic codes and subjects, verifier verdict and blocking verifier categories and subjects as an earlier round. Do not publish a candidate that never passed. Provider transport recovery and schema repair remain separate from this semantic repair allowance.

A repair identity-change ledger uses the shared `IdentityChange` contract and accounts for every candidate-owned record removed, added or changed by the replacement. This includes a changed record whose stable canonical ID is unchanged. Store the normalized ledger with the verified repair response so the accepted transition remains auditable. Synthesis and verification responses carry an empty ledger. Old IDs remain identities in the rejected candidate and the ledger; they are not active aliases in the replacement. The replacement must have complete internal reference closure. A one-to-many split must explicitly redirect each reference because the orchestrator cannot infer which successor it means.

...
Deterministic candidate verification reports every independently observable violation in one ordered finding set. Equal violations are grouped by code and message, and their complete affected-subject sets are retained. A failed prerequisite suppresses only checks that require that prerequisite; it does not stop unrelated checks. Every finding identifies at least one candidate or graph-scope subject. Schema and complete reference validation remain final defense-in-depth gates after the accumulated checks pass.
...
If either verifier rejects the candidate, permit repair requests for that semantic unit up to `semantic_repair_limit`, subject to the configured build budget and deadline. Each repair receives the original graph, the immediately previous rejected candidate, its canonical hash, all deterministic findings and its independent verification report. It returns a complete replacement candidate. Run deterministic and independent semantic verification again on every replacement. Stop early when a replacement produces the same deterministic codes and subjects, verifier verdict and blocking verifier categories and subjects as an earlier round. Do not publish a candidate that never passed. Provider transport recovery and schema repair remain separate from this semantic repair allowance.

After normalizing a repair replacement, the orchestrator derives its identity-change ledger from the exact parent/replacement diff. A same-ID body change becomes `revised`, a removed record becomes `retired`, and a new record becomes `added`. Store that ledger with the verified repair response so the accepted transition remains auditable. Synthesis and verification responses carry an empty ledger. Old IDs remain identities in the rejected candidate and the ledger; they are not active aliases in the replacement. The replacement must have complete internal reference closure, and every active reference must already identify its intended replacement record. Cross-publication domain splits and merges remain the responsibility of deterministic membership reconciliation.

A repair may change a record named by a deterministic or verifier finding and records in its candidate-reference dependency closure. Changing any other prior record is `UNJUSTIFIED_REGRESSION`. The exact parent-candidate hash prevents a repair or cached response from being applied to a different candidate.
...
**Developer extraction measurement:** Run `.venv/bin/python3 scripts/measure-business-domain-extraction.py --repo PATH` to report trace resolution, unresolved call targets by normalized language and UI-interaction population without semantic synthesis. `--read FACTS_JSON` reports from an existing facts artifact, and `--out FACTS_JSON` preserves newly extracted facts for comparison. This script is test and evaluation tooling; it does not add a `speed digest` flag or publish a domain model or repository digest.

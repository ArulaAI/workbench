...
_FENCE = "---"
_MANAGED_PREFIX = "x-workbench-"


def _closing_fence(lines: list):
    """Index of the closing fence, or None when there is no front matter."""
...
def _without_managed(block: list) -> list:
    ...
    for line in block:
        if _is_entry(line.rstrip("\n")):
            ...
            dropping = key.startswith(_MANAGED_PREFIX)
            if dropping:
                continue
        elif dropping:
...
def inject(text: str, extra: dict) -> str:
    ...
    folded scalars valid in the projection.
    """
